...
tg_menu() {
  tg_header
  echo -e "${CYAN}$UI_BOX_TOP${NC}"
  ui_2col "$(ui_menu_num 01 'Setup Bot Token' "$GREEN")" "$(ui_menu_num 02 'Test Kirim Notifikasi' "$GREEN")"
  ui_2col "$(ui_menu_num 03 'Default maxlogin')" "$(ui_menu_num 04 'Default Limit Kuota')"
  ui_2col "$(ui_menu_num 05 'Default Durasi Trial')" "$(ui_menu_num 06 'Toggle Notifikasi' "$PURPLE")"
  echo -e "${CYAN}$UI_BOX_MID${NC}"
  ui_2col "$(ui_menu_num 07 'Matikan Bot Telegram' "$RED")" "$(ui_menu_num 00 'Kembali' "$DIM")"
  echo -e "${CYAN}$UI_BOX_BOT${NC}"
  echo ""
...
do_toggle_notify() {
  ...
  echo -e "  ${CYAN}$LINE${NC}"
  echo -e "  ${GREEN}[01]${NC} Notif Akun Baru   : $([[ "${TG_NOTIFY_CREATE:-1}" == "1" ]] && echo ON || echo OFF)"
  echo -e "  ${GREEN}[02]${NC} Notif Akun Hapus  : $([[ "${TG_NOTIFY_DELETE:-1}" == "1" ]] && echo ON || echo OFF)"
  echo -e "  ${GREEN}[03]${NC} Notif Limit Habis : $([[ "${TG_NOTIFY_LIMIT:-1}" == "1" ]] && echo ON || echo OFF)"
  echo -e "  ${DIM}[00]${NC} Kembali"
  echo ""
