...
do_create_ssh() {
  ...
  echo -ne "  ${YELLOW}Max Login (Device/IP)${NC} [default: ${SESSION_LIMIT_DEFAULT:-2}, 0=unlimited]: "
  ...
  echo -e "  ${YELLOW}Max Login       ${NC}: ${WHITE}$(fmt_limit "$session_limit")${NC}"
...
do_info_ssh() {
  ...
  echo -e "  ${YELLOW}Login Aktif     ${NC}: ${WHITE}$active${NC} / ${WHITE}$(fmt_limit "$slimit")${NC}"
...
do_edit_limit_ssh() {
  sshws_header
  echo ""
  echo -e "  ${WHITE}EDIT MAX LOGIN & KUOTA${NC}"
  echo -e "  ${CYAN}$LINE${NC}"
  echo ""
  echo -ne "  ${YELLOW}Username${NC}: "; read -r username
  local info=$(get_ssh_info "$username")
  ...
  echo -ne "  ${YELLOW}Max Login baru${NC} [saat ini: $(fmt_limit "$cur_slimit")]: "
